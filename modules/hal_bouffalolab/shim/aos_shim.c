 ...
 *
 * AliOS-to-Zephyr shim for the BL606P/BL808 native BLE controller blob
 * (libbtcontroller.a). Implements aos_*, bl_irq_*, and platform stubs
 * mapping them to Zephyr kernel primitives.
 */
...
/* XIP flash base on BL808 */
#define BL808_XIP_BASE 0x58000000U
...
void *aos_malloc(unsigned int size)
{
	...
	if (p == NULL) {
		printk("BLE shim: aos_malloc(%u) FAILED\n", size);
	}
	return p;
}
		     ...
		     int prio)
{
	...
	/*
	 * Use cooperative priority so the BLE task can run when
	 * k_yield is called from the BT host (also cooperative).
	 */

	/* Map to cooperative priority for proper scheduling with BT host */
	zprio = K_PRIO_COOP(8);
	ARG_UNUSED(prio);
