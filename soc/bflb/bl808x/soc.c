...
/*
 * soc_reset_hook: runs from __start (vectors section) BEFORE stack/BSS setup.
 * Must be naked — no stack available at this point.
 *
 * The bootloader leaves machine interrupts enabled, D-cache hot with
 * speculative prefetch (MHINT=0x000c), and branch prediction active.
 * Before mtvec is configured, any pending interrupt causes an unhandled
 * trap. D-cache speculation through SF_CTRL can also conflict with
 * I-cache line fills at certain XIP addresses, hanging the core.
 *
 * This hook immediately:
 *  1. Disables machine interrupts (MIE in mstatus)
 *  2. Disables D-cache, write-back, and branch prediction (MHCR)
 *  3. Clears D-cache speculation hints (MHINT)
 *
 * arch_cache_init() re-enables caches and speculation later after
 * proper flush/invalidate.
 */
void __attribute__((naked, section(".reset.soc_reset_hook"))) soc_reset_hook(void)
{
	__asm__ volatile(
		"csrc mstatus, 8\n"    /* Clear MIE — prevent unhandled interrupts */
		"li t0, 0x1026\n"     /* DE(1)|WB(2)|BPE(5)|L0BTB(12) */
		"csrc 0x7C1, t0\n"    /* MHCR: disable D-cache + branch prediction */
		"csrw 0x7C5, zero\n"  /* MHINT: disable speculative prefetch */
		"ret\n"
	);
}
...
#ifndef MM_GLB_BASE
#define MM_GLB_BASE ((uint32_t)0x30007000U)
#endif
...
/* T-Head E907 MHINT (Machine Hardware Hint) register — CSR 0x7C5.
 * SDK sets 0x000c: bit 2 = DPLD (D-cache prefetch), bit 3 = AMR (auto memory request).
 * Without this, D-cache prefetch from flash XIP is address-pattern-dependent
 * and certain code layouts cause hangs.
 */
#define THEAD_MHINT_DPLD BIT(2)
#define THEAD_MHINT_AMR  BIT(3)
...
static void system_sysmap_init(void)
{
	...
	sys_write32(BL808_OCRAM_BUSREMAP_CACHEABLE_BASE >> SYSMAP_BASE_SHIFT,
		    base + SYSMAP_ADDR_OFFSET);
	sys_write32(SYSMAP_ATTR_STRONG_ORDER, base + SYSMAP_FLAGS_OFFSET);
	base += SYSMAP_ENTRY_OFFSET;

	/* 5: 0x62020000 ~ 0x62058000: CB (OCRAM + WRAM cacheable) */
	sys_write32(BL808_WRAM_BUSREMAP_CACHEABLE_END >> SYSMAP_BASE_SHIFT,
		    base + SYSMAP_ADDR_OFFSET);
	sys_write32(SYSMAP_ATTR_CACHE_ABLE | SYSMAP_ATTR_BUFFER_ABLE, base + SYSMAP_FLAGS_OFFSET);
	base += SYSMAP_ENTRY_OFFSET;
