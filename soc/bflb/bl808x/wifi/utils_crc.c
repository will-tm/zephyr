 ...
 *
 * CRC utilities — copied from M1s SDK components/utils/src/utils_crc.c
 */
...
uint32_t utils_crc32(void *dataIn, uint32_t len)
{
	...
	for (uint32_t i = 0; i < len; i++) {
		crc = crc32Tab[(crc ^ data[i]) & 0xFFU] ^ (crc >> 8U);
	}
...
{
	for (uint32_t i = 0; i < len; i++) {
		ctx->crc = crc32Tab[(ctx->crc ^ data[i]) & 0xFFU] ^ (ctx->crc >> 8U);
	}
}
