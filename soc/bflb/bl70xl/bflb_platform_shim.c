...
/*
 * IRQ handler/context tables — referenced by ROM extension objects
 * (rom_hal_ext.o, rom_freertos_ext.o) via the `handler_list` symbol
 * and the GP-relative `bl_irq_handler_list` / `bl_irq_ctx_list` pointers.
 */
#define BL_IRQ_TABLE_SIZE (16 + 64) /* IRQ_NUM_BASE + 64 external IRQs */

void (*handler_list[BL_IRQ_TABLE_SIZE])(void);
void *irq_ctx_table[BL_IRQ_TABLE_SIZE];
...
static void bl_irq_bridge_isr(const void *arg)
{
	...
	if (irqnum >= 0 && irqnum < BL_IRQ_TABLE_SIZE && handler_list[irqnum]) {
		handler_list[irqnum]();
	}
}
...
{
	if (irqnum < 0 || irqnum >= BL_IRQ_TABLE_SIZE) {
		return;
	}
...
/*
 * Busy-wait microsecond delay, called by the BLE controller blob.
 * Uses the RISC-V machine timer (mtime) for cycle-accurate timing.
 * The mtimer runs at a known frequency set by the clock driver.
 */
void BL702L_Delay_US(uint32_t cnt)
{
	uint32_t ticks_per_us = sys_clock_hw_cycles_per_sec() / 1000000;
	uint32_t lo, hi, hi2;
	uint64_t start, now;
	...
	uint64_t target = (uint64_t)cnt * ticks_per_us;
...
void BL702L_Delay_MS(uint32_t cnt)
{
	...
	uint32_t remain = cnt & 0x3FF;

	if (remain) {
		BL702L_Delay_US(remain * 1000);
	}
}
...
uint32_t GLB_Set_BLE_CLK(uint8_t enable)
{
	...
	tmpVal = sys_read32(GLB_BASE + GLB_CLK_CFG1_OFFSET);
	if (enable) {
		tmpVal |= (1U << GLB_BLE_EN_POS);
	} else {
		tmpVal &= GLB_BLE_EN_UMSK;
	}
	sys_write32(tmpVal, GLB_BASE + GLB_CLK_CFG1_OFFSET);

	tmpVal = sys_read32(GLB_BASE + GLB_CGEN_CFG2_OFFSET);
	if (enable) {
		tmpVal |= (1U << GLB_CGEN_S301_POS);
	} else {
		tmpVal &= GLB_CGEN_S301_UMSK;
	}
	sys_write32(tmpVal, GLB_BASE + GLB_CGEN_CFG2_OFFSET);
...
/*
 * Copy n 32-bit words from src to dst.
 * Note: n is a word count, not byte count.
 */
...
void rom_bl_rtc_trigger_xtal_cnt_32k(void)
{
	...
	if (((tmpVal & HBN_F32K_SEL_MSK) >> HBN_F32K_SEL_POS) == 1) {
		return;
	}

	/* Clear done flag */
	tmpVal = sys_read32(GLB_BASE + GLB_XTAL_DEG_32K_OFFSET);
	tmpVal |= (1U << GLB_CLR_XTAL_CNT_32K_DONE_POS);
	sys_write32(tmpVal, GLB_BASE + GLB_XTAL_DEG_32K_OFFSET);

	/* Trigger measurement */
	tmpVal = sys_read32(GLB_BASE + GLB_XTAL_DEG_32K_OFFSET);
	tmpVal |= (1U << GLB_XTAL_CNT_32K_SW_TRIG_PS_POS);
	sys_write32(tmpVal, GLB_BASE + GLB_XTAL_DEG_32K_OFFSET);
}
...
uint16_t rom_bl_rtc_get_xtal_cnt_32k_counter(void)
{
	...
	tmpVal = sys_read32(PDS_BASE + PDS_XTAL_CNT_32K_OFFSET);

	/* If measurement in progress, wait for done */
	if ((tmpVal & PDS_XTAL_CNT_32K_PROCESS_MSK) != 0) {
		do {
			tmpVal = sys_read32(PDS_BASE + PDS_XTAL_CNT_32K_OFFSET);
		} while ((tmpVal & PDS_XTAL_CNT_32K_DONE_MSK) == 0);
	}

	return (uint16_t)((tmpVal & PDS_RO_XTAL_CNT_32K_CNT_MSK) >> PDS_RO_XTAL_CNT_32K_CNT_POS);
}
...
uint32_t rom_bl_rtc_32k_to_32m(uint32_t cycles)
{
	...
	tmpVal = sys_read32(PDS_BASE + PDS_XTAL_CNT_32K_OFFSET);
	cnt = (uint16_t)((tmpVal & PDS_RO_XTAL_CNT_32K_CNT_MSK) >> PDS_RO_XTAL_CNT_32K_CNT_POS);
	res = (uint16_t)((tmpVal & PDS_RO_XTAL_CNT_32K_RES_MSK) >> PDS_RO_XTAL_CNT_32K_RES_POS);

	return cycles * cnt + cycles * res / 64;
}

/* ===== ROM RTC counter shims ===== */
