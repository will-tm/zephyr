...
static void c906_enable_cache(void)
{
	unsigned long val;

	__asm__ volatile("csrr %0, 0x7C1" : "=r"(val));
	val |= THEAD_MHCR_IE | THEAD_MHCR_DE | THEAD_MHCR_WB | THEAD_MHCR_WA | THEAD_MHCR_RS |
	       THEAD_MHCR_BPE | THEAD_MHCR_BTB;
	__asm__ volatile("csrw 0x7C1, %0" : : "r"(val));
}

static void c906_enable_prefetch(void)
{
	unsigned long val;
...
{
	/* Write mapbaddr (CSR 0xfc1) to XRAM 0x40000108 for debugging */
