 ...
 *   0x28000000 - 0x29000000  BLE Exchange Memory                   → B
 *   0x29000000 - 0x3EF80000  Peripherals                           → SO
 *   0x3EF80000 - 0x40000000  MM L2SRAM + VRAM (DSP DRAM)          → B
 *   0x40000000 - 0x50000000  Gap                                   → SO
...
static void system_sysmap_init(void)
{
	...
	/* 1: 0x28000000 ~ 0x29000000: B (bufferable, BLE Exchange Memory) */
	...
	/* 2: 0x29000000 ~ 0x3EF80000: SO (peripherals) */
	sys_write32(0x3EF80000U >> SYSMAP_BASE_SHIFT, base + SYSMAP_ADDR_OFFSET);
	sys_write32(SYSMAP_ATTR_STRONG_ORDER, base + SYSMAP_FLAGS_OFFSET);
	base += SYSMAP_ENTRY_OFFSET;
#else
	/* 1: 0x28000000 ~ 0x3EF80000: SO (peripherals, no BLE EM) */
	sys_write32(0x3EF80000U >> SYSMAP_BASE_SHIFT, base + SYSMAP_ADDR_OFFSET);
	sys_write32(SYSMAP_ATTR_STRONG_ORDER, base + SYSMAP_FLAGS_OFFSET);
	base += SYSMAP_ENTRY_OFFSET;
#endif

	/* N: 0x3EF80000 ~ 0x40000000: B (MM L2SRAM + VRAM, bufferable only) */
	sys_write32(0x40000000U >> SYSMAP_BASE_SHIFT, base + SYSMAP_ADDR_OFFSET);
	sys_write32(SYSMAP_ATTR_BUFFER_ABLE, base + SYSMAP_FLAGS_OFFSET);
	base += SYSMAP_ENTRY_OFFSET;

	/* N+1: 0x40000000 ~ 0x50000000: SO (gap) */
	sys_write32(0x50000000U >> SYSMAP_BASE_SHIFT, base + SYSMAP_ADDR_OFFSET);
	sys_write32(SYSMAP_ATTR_STRONG_ORDER, base + SYSMAP_FLAGS_OFFSET);
	base += SYSMAP_ENTRY_OFFSET;

	/* N+2: 0x50000000 ~ 0x54000000: CB (PSRAM data bus) */
	...
	/* N+3: 0x54000000 ~ 0x5C000000: C (gap + flash XIP) */
	...
	/* N+4: 0x5C000000 ~ 0x62058000: CB (flash2 gap + OCRAM/WRAM cacheable) */
	sys_write32(BL808_WRAM_CACHEABLE_END >> SYSMAP_BASE_SHIFT,
		    base + SYSMAP_ADDR_OFFSET);
	sys_write32(SYSMAP_ATTR_CACHE_ABLE | SYSMAP_ATTR_BUFFER_ABLE, base + SYSMAP_FLAGS_OFFSET);
	base += SYSMAP_ENTRY_OFFSET;

	/* N+5: 0x62058000 ~ 0xFFFFF000: SO (gap + CLIC/sysmap) */
...
#if defined(CONFIG_BT_BFLB_BL808)
	/* Pre-set EM_SEL before blob runs.
	 * Critical: btble_ke_mem_init runs before ble_rf_init in rwip_init.
	 * Without EM enabled, ke_mem_init writes garbage heap metadata.
	 * 0x0F = 32KB EM, 128KB WRAM. Blob may adjust later.
	 */
	uint32_t tmp = sys_read32(GLB_BASE + GLB_SRAM_CFG3_OFFSET);
	tmp &= ~GLB_EM_SEL_MSK;
	tmp |= (0x0F << GLB_EM_SEL_POS);
	sys_write32(tmp, GLB_BASE + GLB_SRAM_CFG3_OFFSET);

	/* Enable WiFi PHY clock gate (shared RF for BLE).
	 * CGEN_CFG0 bit 7 may be TZC-protected after boot2 — try here
	 * at earliest opportunity.
	 */
	sys_write32(sys_read32(GLB_BASE + 0x580) | BIT(7), GLB_BASE + 0x580);
#endif
