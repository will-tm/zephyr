...
static void bflb_on_scan_done(uint8_t status)
{
	...
	/* Firmware has sent all results and SCANU_START_CFM — finish now.
	 * Cancel any pending delayed work from bflb_on_scan_result.
	 */
	k_work_cancel_delayable(&data->scan_done_work);
	bflb_finish_scan(data);
}
			  ...
			  scan_result_cb_t cb)
{
	...
	LOG_INF("Starting scan (mode=%u duration=%u timeout=%u)", scan_mode, duration_scan_us,
		data->scan_timeout_ms);
	ret = bl_main_scan(NULL, NULL, 0, NULL, NULL, scan_mode, duration_scan_us);
	LOG_INF("bl_main_scan returned %d", ret);
	if (ret) {
		data->scan_cb = NULL;
		data->scan_timeout_ms = 0;
		data->scan_deadline_ms = 0;
...
static int bflb_wifi_connect(const struct device *dev, struct wifi_connect_req_params *params)
{
	...
	LOG_INF("connect params: security=%u psk_len=%d ssid_len=%u", params->security, psk_len,
		params->ssid_length);

	/* Save passphrase for WPA supplicant — firmware's internal copy
	 * may be corrupted due to sm_connect_req struct padding mismatch. */
	{
		extern void bl_wifi_save_passphrase(const char *psk, int len);
		bl_wifi_save_passphrase((const char *)psk, psk_len);
	}
...
extern int bl_output_raw(const uint8_t *frame, uint16_t len, uint8_t vif_idx, uint8_t sta_idx);
...
static void bflb_wifi_iface_init(struct net_if *iface)
{
	...
	/* Register RX interface for firmware frame delivery */
	{
		extern void bl_wifi_set_rx_iface(struct net_if *);
...
static int bflb_wifi_init(const struct device *dev)
{
	...
	LOG_INF("Starting WiFi firmware...");
	...
	LOG_INF("WiFi firmware started, bl_hw=%p", bl_hw);
	bflb_wifi_data_inst.initialized = true;
	return 0;
}
