...
/// Tasks types.
/// The firmware blob uses -fshort-enums, so ke_task_id_t is uint8_t in the
/// ABI.  We typedef it to uint8_t so struct layouts (ipc_e2a_msg, lmac_msg)
/// match the firmware's packed format.
enum wifi_fw_task_id
{
    TASK_NONE = 0xFF,
...
/// Message IDs — firmware blob uses -fshort-enums so ke_msg_id_t is uint16_t.
enum wifi_fw_event_id
{
    /// RESET Request.
