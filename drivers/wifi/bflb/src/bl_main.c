...
/* Local type defs — can't include bl_main.h due to struct conflicts */
...
#if defined(CONFIG_SOC_SERIES_BL808) && !defined(WIFI_IRQn)
/* BL808 hal_bouffalolab bflb_soc.h has no IRQn_Type enum (unlike
 * BL60x/BL70x/BL61x). Define here until the hal is updated.
 */
#define WIFI_IRQn     70
#define WIFI_IPC_IRQn 79
#endif
...
/* struct netif, pbuf, and related types are in wifi_cfg.h */
...
int __attribute__((weak)) bl_main_apm_start_old(char *ssid, char *password, int channel, uint8_t vif_index,
...
int __attribute__((weak)) bl_main_apm_stop_old(uint8_t vif_index)
{
...
static int cfg80211_init(struct bl_hw *bl_hw)
{
	...
	LOG_INF("waiting 500ms for firmware ke_task init...");
	k_msleep(500);
	LOG_INF("calling bl_send_me_config_req");
	bl_send_me_config_req(bl_hw);

	/* Initialize channel list before sending channel config */
	bl_msg_update_channel_cfg("CN");
...
int bl_main_rtthread_start(struct bl_hw **bl_hw)
{
	...
	/* Don't install stdout hook — blob's printf spam goes to /dev/null.
	 * Our printf() override in wifi_stubs.c is a no-op. */
...
/* AP mode stubs (matching new bl_main.h signatures) */
