...
int tcpip_stack_input(void *swdesc, uint8_t status, void *hwhdr, unsigned int msdu_offset, struct wifi_pkt *pkt, uint8_t extra_status)
{
    ...
    /* Deliver frame to Zephyr net stack (bypass LWIP pbuf system) */
    if (!sniffer) {
        extern struct net_if *bl_wifi_get_rx_iface(void);
        struct net_if *iface = bl_wifi_get_rx_iface();
        uint16_t frame_len = pkt->len[0] - msdu_offset;

        if (iface && frame_len > 0) {
            struct net_pkt *npkt = net_pkt_rx_alloc_with_buffer(
                iface, frame_len, AF_UNSPEC, 0, K_NO_WAIT);
            if (npkt) {
                net_pkt_write(npkt, (uint8_t *)pkt->pkt[0] + msdu_offset, frame_len);
                /* Append chained segments */
                for (int seg = 1; seg < 4 && pkt->len[seg] > 0; seg++) {
                    net_pkt_write(npkt, (uint8_t *)pkt->pkt[seg], pkt->len[seg]);
                }
                net_pkt_cursor_init(npkt);
                if (net_recv_data(iface, npkt) < 0) {
                    net_pkt_unref(npkt);
                }
            }
        }
...
extern void bl_rx_e2a_handler(void *arg);

u8 bl_msgind(void *pthis, void *hostid)
{
    struct bl_hw *bl_hw = (struct bl_hw *)pthis;

    /* Route to bl_rx_e2a_handler which has the full dispatch table */
    if (bl_hw->cmd_mgr.msgind) {
        extern void bl_rx_handle_msg(struct bl_hw *bl_hw, struct ipc_e2a_msg *msg);
        /* The E2A MSG is in the IPC shared memory. Read the buffer. */
        struct ipc_host_env_tag *env = bl_hw->ipc_env;
        uint8_t idx = env->ipc_host_msge2a_idx;
        struct ipc_e2a_msg *msg = (struct ipc_e2a_msg *)env->ipc_host_msgbuf_array[idx].hostid;

        if (msg && msg->pattern == IPC_MSGE2A_VALID_PATTERN) {
            msg->pattern = 0;
            bl_rx_handle_msg(bl_hw, msg);
        }
        env->ipc_host_msge2a_idx = (idx + 1) % IPC_MSGE2A_BUF_CNT;
    }
    return 0;
}
...
int bl_ipc_init(struct bl_hw *bl_hw, struct ipc_shared_env_tag *ipc_shared_mem)
{
    ...
    cb.recv_msg_ind    = NULL; /* TODO: enable when bl_rx.c E2A handling is stable */
