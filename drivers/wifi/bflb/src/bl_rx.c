...
extern void bl_wifi_notify_scan_done(uint8_t status);
      ...
      struct bl_cmd *cmd, struct ipc_e2a_msg *msg)
{
    ...
    /* Notify Zephyr wifi_mgmt that scan is complete */
    struct scanu_start_cfm *cfm = (struct scanu_start_cfm *)msg->param;
    bl_wifi_notify_scan_done(cfm->status);
...
extern void bl_wifi_notify_scan_result(const uint8_t *payload, uint16_t len,
...
static int bl_rx_scanu_result_ind(struct bl_hw *bl_hw, struct bl_cmd *cmd, struct ipc_e2a_msg *msg)
{
    ...
    uint16_t payload_len = (msg->param_len > sizeof(struct scanu_result_ind))
                         ? (msg->param_len - sizeof(struct scanu_result_ind))
                         : ind->length;

    /* Deliver to Zephyr wifi_mgmt scan callback */
    bl_wifi_notify_scan_result((const uint8_t *)ind->payload, payload_len, ind);

    /* Also deliver to legacy beacon/probe callbacks if registered */
                                         ...
                                         struct ipc_e2a_msg *msg)
{
    ...
    /* Notify Zephyr wifi_mgmt */
    {
        extern void bl_wifi_notify_connect_ind(const struct sm_connect_ind *);
        bl_wifi_notify_connect_ind(ind);
    }
                                            ...
                                            struct ipc_e2a_msg *msg)
{
    ...
    /* Notify Zephyr wifi_mgmt */
    {
        extern void bl_wifi_notify_disconnect_ind(const struct sm_disconnect_ind *);
        bl_wifi_notify_disconnect_ind(ind);
    }
...
{
    if (!wifi_hw.cmd_mgr.msgind) {
        return; /* cmd_mgr not initialized yet */
    }
    struct ipc_e2a_msg *msg = (struct ipc_e2a_msg*)arg;
    uint16_t task = MSG_T(msg->id);
    uint16_t idx = MSG_I(msg->id);
    msg_cb_fct cb = NULL;

    if (task < (sizeof(msg_hdlrs) / sizeof(msg_hdlrs[0])) && msg_hdlrs[task]) {
        cb = msg_hdlrs[task][idx]; /* may be NULL for CFM msgs */
    }

    wifi_hw.cmd_mgr.msgind(&wifi_hw.cmd_mgr, msg, cb);
}
