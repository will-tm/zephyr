...
void bl_wifi_notify_scan_result(const uint8_t *payload, uint16_t len,
				const struct scanu_result_ind *ind)
{
	if (scan_result_cb) {
		scan_result_cb(payload, len, ind);
	}
}

void bl_wifi_notify_scan_done(uint8_t status)
{
	if (scan_done_cb) {
		scan_done_cb(status);
	}
...
void bl_wifi_notify_connect_ind(const struct sm_connect_ind *ind)
{
	if (connect_ind_cb) {
		connect_ind_cb(ind);
	}
...
void bl_wifi_notify_disconnect_ind(const struct sm_disconnect_ind *ind)
{
	if (disconnect_ind_cb) {
		disconnect_ind_cb(ind);
	}
}

void bl_tx_cntrl_link_up(void)
{
}
...
/* Saved passphrase from bflb_wifi_connect — firmware's internal copy
 * may have wrong offset due to struct padding mismatch. */
static char saved_passphrase[65];

void bl_wifi_save_passphrase(const char *psk, int len)
{
	memset(saved_passphrase, 0, sizeof(saved_passphrase));
	if (psk && len > 0 && len < (int)sizeof(saved_passphrase)) {
		memcpy(saved_passphrase, psk, len);
	}
}
...
static void wpa_sta_connect_impl(wifi_connect_parm_t *p)
{
	if (p) {
		LOG_INF("wpa_sta_connect: ssid=%.*s proto=%u sta=%u "
			...
			p->ssid.len, p->ssid.ssid, p->proto, p->sta_idx, p->bssid[0], p->bssid[1],
			p->bssid[2], p->bssid[3], p->bssid[4], p->bssid[5]);
		/* Override firmware's passphrase with our saved copy */
		if (saved_passphrase[0]) {
			memcpy(p->passphrase, saved_passphrase, sizeof(p->passphrase));
		}
		wpa_sm_init(p);
	}
}
...
static int wpa_sta_rx_eapol_impl(uint8_t *src, uint8_t *buf, uint32_t len)
{
	...
	LOG_INF("EAPOL RX: src=%02x:%02x:%02x:%02x:%02x:%02x len=%u", src[0], src[1], src[2],
...
/* Stubs for new pinevoice firmware blob symbols */
#include <zephyr/kernel.h>
...
{
	struct utils_list_hdr *elt = list->first;
	if (elt) {
		list->first = elt->next;
		if (!list->first) {
			list->last = NULL;
...
void utils_list_push_back(struct utils_list *list, struct utils_list_hdr *elt)
{
	elt->next = NULL;
	if (list->last) {
		list->last->next = elt;
...
/* Additional stubs for pinevoice firmware blob */
#include "utils_list.h"
#include <stdarg.h>
...
/* LWIP pbuf stubs */
struct pbuf *pbuf_alloced_custom(int layer, uint16_t length, int type, void *custom, void *payload,
...
int pbuf_take(struct pbuf *buf, const void *data, uint16_t len)
{
	return -1;
}

/* WiFi callbacks - will be implemented in bflb_wifi_drv.c */
void __attribute__((weak)) bl_rx_pkt_cb(void *env, void *pkt)
{
}
void __attribute__((weak)) bl_tx_cfm(void *env, uint32_t *status)
{
}
static struct net_if *wifi_rx_iface;
...
struct net_if *bl_wifi_get_rx_iface(void)
{
	return wifi_rx_iface;
}
int __attribute__((weak)) wifi_mgmr_ext_dump_needed(void)
{
	return 0;
}
int __attribute__((weak)) bl_output(void *hw, int is_sta, void *p, void *cfm)
{
	return 0;
}
int wpa_parse_wpa_ie_wrapper(void *wpa_ie, int wpa_ie_len, void *data)
{
	return -1;
}

void netifapi_netif_set_link_down(void *n)
{
}
void netifapi_netif_set_addr(void *n, void *a, void *b, void *c)
{
}
void netifapi_netif_set_link_up(void *n)
{
}
...
void bl_tx_cntrl_link_down(void)
{
}
