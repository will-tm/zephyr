...
static int bflb_tsen_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	ret = adc_read(cfg->adc.dev, &data->seq);
	if (ret < 0) {
		return ret;
	}

	data->temp_mc = tsen_compute_temp_mc(data->sample, data->tsen_offset);

	LOG_DBG("delta=%u offset=%u temp=%d.%03d degC", data->sample, data->tsen_offset,
...
static int bflb_tsen_init(const struct device *dev)
{
	...
	data->seq.buffer = &data->sample;
	data->seq.buffer_size = sizeof(data->sample);
	(void)adc_sequence_init_dt(&cfg->adc, &data->seq);
