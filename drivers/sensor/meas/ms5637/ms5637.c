...
static bool ms5637_prom_crc_valid(uint16_t *prom)
{
	...
	expected_crc = (uint8_t)(prom[MS5637_PROM_CRC_IDX] >> 12);
...
static int ms5637_read_prom_word(const struct device *dev, uint8_t addr,
				 uint16_t *val)
{
 ...
 */
static int ms5637_convert_and_read(const struct device *dev, uint8_t conv_cmd,
				   uint8_t delay_ms, uint32_t *adc_val)
{
	const struct ms5637_config *cfg = dev->config;
	uint8_t cmd;
	...
	*adc_val = ((uint32_t)buf[0] << 16) | ((uint32_t)buf[1] << 8) |
		   (uint32_t)buf[2];
 ...
 */
static void ms5637_compensate(struct ms5637_data *data,
			      ...
			      uint32_t adc_pressure)
{
	...
	data->pressure = (int32_t)((SENS * (int64_t)adc_pressure / (1LL << 21) -
				    OFF) / (1LL << 15));
}

static int ms5637_sample_fetch(const struct device *dev,
			       enum sensor_channel chan)
{
	...
	ret = ms5637_convert_and_read(dev, data->pressure_conv_cmd,
				      data->pressure_conv_delay, &adc_pressure);
	if (ret < 0) {
		LOG_ERR("Pressure conversion failed: %d", ret);
		return ret;
	}

	ret = ms5637_convert_and_read(dev, data->temperature_conv_cmd,
...
static int ms5637_channel_get(const struct device *dev,
			      enum sensor_channel chan,
			      struct sensor_value *val)
{
	...
	case SENSOR_CHAN_AMBIENT_TEMP:
		/* Internal temperature is in centidegrees Celsius */
		val->val1 = data->temperature / 100;
		val->val2 = (data->temperature % 100) * 10000;
		break;
	case SENSOR_CHAN_PRESS:
		/*
		 * Internal pressure is mbar * 100 (i.e. Pascals).
		 * SENSOR_CHAN_PRESS unit is kPa: 1000 Pa = 1 kPa.
		 */
...
static int ms5637_init(const struct device *dev)
{
	...
	ret = ms5637_attr_set(dev, SENSOR_CHAN_AMBIENT_TEMP,
			      SENSOR_ATTR_OVERSAMPLING, &osr);
	if (ret < 0) {
		return ret;
	}
...
#define MS5637_INIT(inst)                                                      \
	...
	SENSOR_DEVICE_DT_INST_DEFINE(inst, ms5637_init, NULL,                  \
				     &ms5637_data_##inst,                      \
