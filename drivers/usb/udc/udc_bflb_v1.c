...
struct udc_bflb_v1_data {
	...
	/* Diagnostic: tracks unserved SETUP requests */
	uint8_t setup_diag[8]; /* previous SETUP bytes (ISR-written) */
	bool setup_pending; /* true from SETUP_DONE until data loaded or STALL */
	uint32_t rend_count;  /* REND interrupt counter (diagnostic) */
	uint32_t setup_count; /* SETUP_DONE counter (diagnostic) */
};
...
static void ep0_set_ready(const mm_reg_t base)
{
	 ...
	 * the thread is ready.  NACK_OUT is explicitly cleared only
	 * when arming for status-OUT (ctrl_feed_dout) or by the ISR
...
static void bflb_prep_ep0_rx(const struct device *dev)
{
	...
	ep_wait_not_busy(config->base, 0);
	ep0_set_ready(config->base);
}
...
/* Allocate and feed a buffer for control OUT data stage */
static int ctrl_feed_dout(const struct device *dev, const size_t length)
{
	const struct udc_bflb_v1_config *const config = dev->config;
	const mm_reg_t base = config->base;
	struct udc_ep_config *const ep_cfg =
	...
	/*
	 * Arm EP0 for OUT and clear NACK_OUT in one register write.
	 * This must be a single write — doing ep0_set_ready() then
	 * ep0_clear_nack_out() as separate RMW operations creates a
	 * window where the ISR could fire (e.g. new SETUP) and modify
	 * USB_CONFIG, which the second RMW would then clobber.
	 */
	ep_wait_not_busy(base, 0);
...
static void handle_evt_setup(const struct device *dev)
{
	...
	if (udc_ctrl_stage_is_data_out(dev)) {
		err = ctrl_feed_dout(dev, udc_data_stage_length(buf));
		if (err != 0) {
			   ...
			   struct udc_ep_config *const ep_cfg)
{
	...
	if (ep_cfg->addr == USB_CONTROL_EP_IN) {
		struct udc_bflb_v1_data *const priv = udc_get_private(dev);
		const bool was_data_in = udc_ctrl_stage_is_data_in(dev);
		int err;

		/* EP0: ISR already handled packet-level work */
		...
		if (was_data_in && !priv->setup_pending) {
			/*
			 * Arm EP0 OUT for the status stage only if no new
			 * SETUP has arrived.  If the ISR already accepted a
			 * new SETUP (setup_pending), the old transfer is
			 * aborted and we must NOT set RDY — doing so would
			 * let the hardware send stale TX FIFO data on the
			 * next IN token (the host is already polling IN for
			 * the new transfer's data phase).
			 */
			err = ctrl_feed_dout(dev, 0);
			if (err != 0) {
		...
		if (udc_ctrl_stage_is_status_out(dev)) {
			...
			if (!was_data_in && !priv->setup_pending) {
				err = ctrl_feed_dout(dev, 0);
				if (err != 0) {
					udc_submit_event(dev, UDC_EVT_ERROR,
							 err);
				}
			}
			return;
		...
		/* Data-phase IN: arm for next packet. */
			    ...
			    struct udc_ep_config *const ep_cfg)
{
	...
	if (ep_cfg->addr == USB_CONTROL_EP_OUT) {
		...
		if (buf == NULL) {
			/*
			 * Status-out ZLP was consumed by ISR before the
			 * IN completion queued the OUT buffer. Silently
			 * ignore; the IN handler will complete the stage.
			 */
			return;
		}
		...
		if (udc_ctrl_stage_is_status_out(dev)) {
			udc_ctrl_submit_status(dev, buf);
			/* ISR already cleared NACK_OUT when the
...
static void reinit_ep0(const struct device *dev)
{
	...
	/* Configure EP0: SW control mode, 64-byte MPS, NAK IN, accept SETUP.
	 * NACK_OUT is NOT set — it blocks SETUP tokens unconditionally.
	 * NACK_IN is set to NAK stray IN tokens until we have data ready.
	 */
	regval = sys_read32(base + USB_CONFIG_OFFSET);
	regval &= ~USB_STS_USB_EP0_SW_RDY;
			       ...
			       const uint8_t ep_idx)
{
	...
	if (ep_idx == 0) {
		...
		/*
		 * Set NAK so the host receives NAK responses (not silence)
		 * while the thread processes the completion and re-arms EP0.
		 */
		ep0_set_nak(base);

		if (!priv->ep0_ctrl_in || !priv->ep0_has_data) {
			/*
			 * Status-IN complete (no-data or control-OUT).
			 * Clear NACK_OUT so the next SETUP is accepted.
			 */
				...
				const uint8_t ep_idx)
{
	...
	if (ep_idx == 0) {
		...
		if (priv->ep0_ctrl_in) {
			/*
			 * Status-OUT of a control-IN transfer complete.
			 * Clear NACK_OUT so the next SETUP is accepted.
			 */
			ep0_clear_nack_out(base);
		}
...
static void udc_bflb_v1_isr(const struct device *dev)
{
	...
	if (sts & USB_EP0_SETUP_DONE_INT) {
		...
		if (ep_get_rxcount(base, 0) == 8) {
			...
			memcpy(priv->setup_diag, priv->setup, 8);
	 ...
	 * For control-IN transfers this allows the host's status-OUT
	 * ZLP to be accepted before the thread queues a buffer — the
	 * ISR silently drops the ZLP (no buffer), and the subsequent
	 * ctrl_feed_dout leaves a stale RDY that gets consumed by the
	 * next transfer's IN token, sending a ZLP from an empty FIFO.
	 *
	 * NACK_OUT is now managed exclusively by the DONE handlers:
	 *  - isr_handle_in_done: clears after status-IN (no-data /
	 *    control-OUT completion)
	 *  - isr_handle_out_done: clears after status-OUT (control-IN
	 *    completion)
	 *  - ep_set_halt: clears so next SETUP is accepted after STALL
	 * The thread clears NACK_OUT via ctrl_feed_dout when ready for
	 * OUT data or status-OUT phases.
	 */
	if (sts & USB_REND_INT) {
		priv->rend_count++;
	}
				 ...
				 struct udc_ep_config *const ep_cfg)
{
	...
	/* Enable DONE interrupt for this endpoint */
				   ...
				   struct udc_ep_config *const ep_cfg)
{
	...
	if (ep_idx == 0) {
		...
		regval |= USB_CR_USB_EP0_SW_STALL;
		/*
		 * Clear NACK_OUT so the next SETUP can be accepted after
		 * the STALL completes.  The hardware sends STALL on the
		 * next IN/OUT token (RDY is armed), and REND should then
		 * fire and also clear NACK_OUT.  But if REND is lost
		 * (e.g. absorbed by a simultaneous SETUP), NACK_OUT
		 * stays at 1 and wedges the device.  Clearing it here
		 * is safe because the host won't send a new SETUP until
		 * after the STALL response completes the current transfer.
		 */
		regval &= ~USB_CR_USB_EP0_SW_NACK_OUT;
		sys_write32(regval, base + USB_CONFIG_OFFSET);
		irq_unlock(key);
