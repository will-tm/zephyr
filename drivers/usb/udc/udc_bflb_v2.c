...
/* Undocumented VDMA abort bit in VDMAFxPS1 registers */
#define USB_BFLB_V2_VDMA_ABORT (1U << 3)

/* G1 interrupt: OUT+SPK pair mask for a FIFO (1-based) */
#define USB_BFLB_V2_G1_OUT_SPK(fifo) (3U << (2U * ((fifo) - 1U)))
/* G1 interrupt: IN completion bit for a FIFO (1-based) */
#define USB_BFLB_V2_G1_IN_INT(fifo)  (1U << (15U + (fifo)))

/* Write-1-to-clear masks for interrupt status groups */
#define USB_BFLB_V2_ISG2_ALL       0x3FFU
#define USB_BFLB_V2_VDMA_CMPLT_ALL 0x1FU

/* Mask upper bits [31:5], unmask lower bits [4:0] */
#define USB_BFLB_V2_MISG_UPPER 0xFFFFFFE0U

/* EPMAP: all endpoints disconnected (0xF = no EP per nibble) */
#define USB_BFLB_V2_EPMAP0_NONE 0xFFFFFFFFU
#define USB_BFLB_V2_EPMAP1_NONE 0xFFU
...
struct udc_bflb_v2_data {
	...
	/* SOF emulation: controller has no SOF interrupt, so dispatch
	 * UDC_EVT_SOF from a periodic timer instead.  dev_back is the
	 * cached back pointer used inside the timer callback.
	 */
	struct k_timer sof_timer;
	const struct device *dev_back;
#endif
};
...
enum udc_bflb_v2_ev_type {
	...
	/* SETUP packet received, deliver to USB stack */
	UDC_BFLB_V2_EVT_SETUP,
	/* CX VDMA complete (control data IN or OUT) */
	UDC_BFLB_V2_EVT_CTRL_DONE,
...
#if defined(CONFIG_UDC_ENABLE_SOF)
static void udc_bflb_v2_sof_timer_cb(struct k_timer *t)
{
	struct udc_bflb_v2_data *priv =
		CONTAINER_OF(t, struct udc_bflb_v2_data, sof_timer);

	udc_submit_sof_event(priv->dev_back);
}

static void udc_bflb_v2_sof_start(const struct device *const dev)
{
	...
	priv->dev_back = dev;
	period = (cfg->speed_idx == UDC_BUS_SPEED_HS) ? K_USEC(125)
						      : K_USEC(1000);
	k_timer_start(&priv->sof_timer, period, period);
}

static void udc_bflb_v2_sof_stop(const struct device *const dev)
{
	struct udc_bflb_v2_data *const priv = udc_get_private(dev);
...
udc_bflb_v2_device_speed(const struct device *const dev)
{
	...
	while (sys_timepoint_expired(priv->reset_expiration) == false) {
		k_msleep(1);
	}
...
static bool udc_bflb_v2_fifo_is_double(const struct device *const dev,
				       const uint8_t fifo)
{
	const struct udc_bflb_v2_config *const cfg = dev->config;
	uint32_t tmp;
	...
	if ((tmp & (USB_BLKSZ_F0 << ((fifo - 1U) * USB_BFLB_V2_FX_X_OFFSET))) !=
	    0) {
		return true;
	}
...
static void udc_bflb_v2_ep_ack(const struct device *const dev,
...
static void udc_bflb_v2_fifo_configure(const struct device *const dev,
				       ...
				       uint8_t block_num, const bool enabled)
{
	const struct udc_bflb_v2_config *const cfg = dev->config;
	uint32_t tmp;
	...
	tmp = sys_read32(cfg->base + USB_DEV_FCFG_OFFSET);
	tmp &= ~(USB_BFLB_V2_FX_X_MASK
		 << ((fifo_idx - 1U) * USB_BFLB_V2_FX_X_OFFSET));
	tmp |= (ep_type << ((fifo_idx - 1U) * USB_BFLB_V2_FX_X_OFFSET +
...
static void udc_bflb_v2_ep_setfifo(const struct device *const dev,
				   const uint8_t ep_idx, const uint8_t fifo_idx,
				   const uint8_t ep_dir)
{
	...
	if (ep_idx < 5U) {
		...
		tmp |= ((uint32_t)(fifo_idx - 1U)
			<< ((uint32_t)(ep_idx - 1U) * 8U + ep_dir_bit));
		...
		tmp |= ((uint32_t)(fifo_idx - 1U)
			<< ((uint32_t)(ep_idx - 5U) * 8U + ep_dir_bit));
				   ...
				   const uint8_t fifo_dir)
{
	...
	tmp &= ~(USB_BFLB_V2_FX_X_MASK
		 << ((uint32_t)(fifo_idx - 1U) * USB_BFLB_V2_FX_X_OFFSET));
	tmp |= ((uint32_t)ep_idx
		<< ((uint32_t)(fifo_idx - 1U) * USB_BFLB_V2_FX_X_OFFSET));
...
static void udc_bflb_v2_vdma_startread(const struct device *const dev,
				       const uint8_t fifo_idx,
				       uint8_t *const buf, const uint32_t len)
{
	const struct udc_bflb_v2_config *const cfg = dev->config;
	const uint32_t fifo_off = (fifo_idx - 1U) * USB_BFLB_V2_FX_X_OFFSET;
	uint32_t tmp;
...
/*
 * Work handler: deliver SETUP packet to the USB stack via new API.
 * The setup bytes were already read from hardware in the ISR.
 */
static void udc_bflb_v2_handle_setup(const struct device *const dev)
{
	const struct udc_bflb_v2_config *const cfg = dev->config;
	struct udc_bflb_v2_data *const priv = udc_get_private(dev);
	uint32_t tmp;
	...
	/* Re-enable setup interrupt after processing */
...
/*
 * Work handler: CX VDMA completed (control data IN or OUT).
 * Dequeue the control buffer and submit completion to USB stack.
 */
static void udc_bflb_v2_handle_ctrl_done(const struct device *const dev)
{
	struct udc_bflb_v2_data *const priv = udc_get_private(dev);
	struct net_buf *buf;

	if (priv->ep_is_in[0]) {
		/* DATA IN completed — tell hardware to finish the transfer */
		...
		/* DATA OUT completed — invalidate cache and submit */
...
static uint32_t udc_bflb_v2_ep_remain(const struct device *const dev,
				      const uint8_t fifo_idx)
{
	const struct udc_bflb_v2_config *const cfg = dev->config;
	uint32_t tmp;
				     ...
				     struct udc_ep_config *const ep_cfg)
{
	...
	if (buf == NULL) {
		...
		tmp = sys_read32(cfg->base + USB_DEV_MISG1_OFFSET);
		tmp &= ~USB_BFLB_V2_G1_IN_INT(fifo);
		sys_write32(tmp, cfg->base + USB_DEV_MISG1_OFFSET);
	}
}
					  ...
					  struct udc_ep_config *const ep_cfg)
{
	...
	if (priv->fifo_active[fifo]) {
		...
		if (USB_EP_DIR_IS_OUT(ep_cfg->addr) && priv->ep_is_in[ep_idx]) {
			...
			LOG_DBG("FIFO %u: evicting IN ep 0x%02x for OUT", fifo,
				in_addr);

			/* Mask G1 OUT/SPK BEFORE resetting FIFO to prevent
			 * stale OUT/SPK interrupts from reading garbage.
			 */
			tmp = sys_read32(cfg->base + USB_DEV_MISG1_OFFSET);
			tmp |= USB_BFLB_V2_G1_OUT_SPK(fifo);
			sys_write32(tmp, cfg->base + USB_DEV_MISG1_OFFSET);
			...
			if (in_cfg != NULL && udc_ep_is_busy(in_cfg)) {
				...
				if (remain > 0) {
					/* IN VDMA unfinished: re-arm. */
					udc_bflb_v2_ev_submit(
						dev, in_addr,
						UDC_BFLB_V2_EVT_XFER, K_NO_WAIT);
				} else {
					/* IN VDMA done: dequeue so the buf
					 * isn't re-sent on the next kick.
					 */
				       ...
				       struct udc_ep_config *completed_cfg)
{
	...
	if (first_cfg != NULL && udc_buf_peek(first_cfg) != NULL &&
	    udc_ep_is_busy(first_cfg) == false) {
		udc_bflb_v2_ev_submit(dev, first_cfg->addr, UDC_BFLB_V2_EVT_XFER,
				      K_NO_WAIT);
		return;
	}

	if (second_cfg != NULL && udc_buf_peek(second_cfg) != NULL &&
	    udc_ep_is_busy(second_cfg) == false) {
		udc_bflb_v2_ev_submit(dev, second_cfg->addr,
				      UDC_BFLB_V2_EVT_XFER, K_NO_WAIT);
		return;
	}
#else
	...
	if (udc_buf_peek(completed_cfg) != NULL &&
	    udc_ep_is_busy(completed_cfg) == false) {
...
static void udc_bflb_v2_work_handler(struct k_work *item)
{
	...
	if (unlikely(ep_cfg == NULL && ev->event != UDC_BFLB_V2_EVT_SETUP)) {
		err = -ENODATA;
		LOG_ERR("Invalid Endpoint Configuration in Work Queue");
	} else {
		switch (ev->event) {
		case UDC_BFLB_V2_EVT_SETUP:
			udc_bflb_v2_handle_setup(ev->dev);
			break;
		case UDC_BFLB_V2_EVT_CTRL_DONE:
			udc_bflb_v2_handle_ctrl_done(ev->dev);
			break;
		case UDC_BFLB_V2_EVT_END:
			if (udc_ep_is_busy(ep_cfg)) {
				if (USB_EP_DIR_IS_OUT(ep_cfg->addr)) {
					udc_bflb_v2_out_chunk_done(ev->dev,
								   ep_cfg);
				} else {
					udc_bflb_v2_in_chunk_done(ev->dev,
								  ep_cfg);
				}
				  ...
				  struct net_buf *buf)
{
	...
	if (ep_idx == 0) {
		if (bi->status) {
			/* Status IN ZLP: tell hardware transfer is done */
			udc_bflb_v2_ctrl_ack(dev);
			buf = udc_buf_get(config);
			if (buf != NULL) {
				udc_submit_ep_event(dev, buf, 0);
			}
		} else if (bi->data) {
			if (USB_EP_DIR_IS_IN(config->addr)) {
				/* DATA IN: start CX VDMA write */
				udc_bflb_v2_vdma_startwrite_ctrl(dev, buf->data,
								 buf->len);
			} else {
				/* DATA OUT: start CX VDMA read */
				net_buf_add(buf, buf->size - buf->len);
				udc_bflb_v2_vdma_startread_ctrl(dev, buf->data,
								buf->len);
			}
		}
	} else {
...
#ifdef CONFIG_UDC_BFLB_V2_FIFO_BIDIR
		if (priv->fifo_active[fifo] == false ||
		    (priv->ep_is_in[ep_idx] ? USB_EP_DIR_IS_IN(ep_cfg->addr)
					    : USB_EP_DIR_IS_OUT(ep_cfg->addr))) {
#else
		if (true) {
#endif
			udc_bflb_v2_vdma_stop(dev, fifo);
				 ...
				 struct udc_ep_config *const config)
{
	...
	if (config->mps > USB_BFLB_V2_HSFIFOCAP) {
		if (ep_idx > 2) {
			LOG_DBG("We need to use 2 FIFO per ep if mps > 512 (USB "
				"High Speed)");
			return -ENOTSUP;
		}
		if (ep_idx == 1) {
			udc_bflb_v2_ep_setfifo(dev, ep_idx, 1,
					       USB_BFLB_V2_EP_DIR_IN);
			udc_bflb_v2_ep_setfifo(dev, ep_idx, 1,
					       USB_BFLB_V2_EP_DIR_OUT);
			udc_bflb_v2_fifo_setep(dev, ep_idx, 1,
					       USB_BFLB_V2_FIFO_DIR_BID);
			udc_bflb_v2_fifo_setep(dev, ep_idx, 2,
					       USB_BFLB_V2_FIFO_DIR_BID);
			udc_bflb_v2_fifo_configure(dev, 1, config, 1, true);
			udc_bflb_v2_fifo_configure(dev, 2, config, 1, false);
		} else if (ep_idx == 2) {
			udc_bflb_v2_ep_setfifo(dev, ep_idx, 3,
					       USB_BFLB_V2_EP_DIR_IN);
			udc_bflb_v2_ep_setfifo(dev, ep_idx, 3,
					       USB_BFLB_V2_EP_DIR_OUT);
			udc_bflb_v2_fifo_setep(dev, ep_idx, 3,
					       USB_BFLB_V2_FIFO_DIR_BID);
			udc_bflb_v2_fifo_setep(dev, ep_idx, 4,
					       USB_BFLB_V2_FIFO_DIR_BID);
			udc_bflb_v2_fifo_configure(dev, 3, config, 1, true);
			udc_bflb_v2_fifo_configure(dev, 4, config, 1, false);
		}
	} else {
		udc_bflb_v2_ep_setfifo(dev, ep_idx, ep_idx,
				       USB_BFLB_V2_EP_DIR_IN);
		udc_bflb_v2_ep_setfifo(dev, ep_idx, ep_idx,
				       USB_BFLB_V2_EP_DIR_OUT);
		udc_bflb_v2_fifo_setep(dev, ep_idx, ep_idx,
				       USB_BFLB_V2_FIFO_DIR_BID);
	...
	if (config->mps > USB_BFLB_V2_HSFIFOCAP) {
		...
		if (USB_EP_DIR_IS_OUT(config->addr)) {
			udc_bflb_v2_ep_setfifo(dev, ep_idx, fifo,
					       USB_BFLB_V2_EP_DIR_OUT);
		} else {
			udc_bflb_v2_ep_setfifo(dev, ep_idx, fifo,
					       USB_BFLB_V2_EP_DIR_IN);
		}
		udc_bflb_v2_fifo_setep(dev, ep_idx, fifo,
				       USB_BFLB_V2_FIFO_DIR_BID);
				  ...
				  struct udc_ep_config *const config)
{
	...
	if (config->mps > USB_BFLB_V2_HSFIFOCAP) {
		...
		if (priv->fifo_active[fifo] == false ||
		    (priv->ep_is_in[ep_idx] ? USB_EP_DIR_IS_IN(config->addr)
					    : USB_EP_DIR_IS_OUT(config->addr))) {
			udc_bflb_v2_vdma_stop(dev, fifo);
			udc_bflb_v2_fifo_reset(dev, fifo);
			priv->fifo_active[fifo] = false;
		}
				     ...
				     struct udc_ep_config *const config)
{
	...
	if (ep_idx == 0) {
...
#ifdef CONFIG_UDC_BFLB_V2_FIFO_BIDIR
		fifo = udc_bflb_v2_ep_to_fifo(config);
		priv = udc_get_private(dev);

		opp_addr = USB_EP_DIR_IS_IN(config->addr)
				   ? (USB_EP_DIR_OUT | ep_idx)
				   : (USB_EP_DIR_IN | ep_idx);
		opp_cfg = udc_get_ep_cfg(dev, opp_addr);
		...
		if (opp_cfg != NULL) {
			udc_ep_set_busy(opp_cfg, false);
		}

		udc_bflb_v2_work_handler_xfer(dev, config);
		return 0;
#endif
		udc_bflb_v2_ev_submit(dev, config->addr, UDC_BFLB_V2_EVT_XFER,
				      K_NO_WAIT);
...
static int udc_bflb_v2_host_wakeup(const struct device *const dev)
{
	...
	if (udc_is_suspended(dev) == false) {
		return -EAGAIN;
	}

	/* Enable remote-wakeup gating in the controller, drive K-state on the
	 * upstream port from the PDS, hold for the USB-spec 1-15 ms window,
	 * then release.  The host responds with bus resume which fires
	 * USB_RESM_INT and clears the suspended state.
	 */
...
static int udc_bflb_v2_enable(const struct device *const dev)
{
	...
	sys_write32(UINT32_MAX, cfg->base + USB_DEV_ISG0_OFFSET);
	/* Clear irqs group 1: FIFO */
	sys_write32(UINT32_MAX, cfg->base + USB_DEV_ISG1_OFFSET);
	/* Clear irqs group 2: device */
	sys_write32(USB_BFLB_V2_ISG2_ALL, cfg->base + USB_DEV_ISG2_OFFSET);
	/* Clear irqs group 3: DMA */
	sys_write32(UINT32_MAX, cfg->base + USB_DEV_ISG3_OFFSET);
	...
	sys_write32(UINT32_MAX, cfg->base + USB_DEV_MISG1_OFFSET);

	/* G2: unmask bits [4:0] = USBRST, SUSP, RESM, ISO_ERR, ISO_ABORT
	 *     + bit 6 = RX0BYTE (OUT ZLP received)
	 */
	sys_write32(USB_BFLB_V2_MISG_UPPER & ~USB_MRX0BYTE_INT,
		    cfg->base + USB_DEV_MISG2_OFFSET);

	/* G3: unmask bits [4:0] = VDMA completion for CXF + FIFO 0-3 */
	sys_write32(USB_BFLB_V2_MISG_UPPER, cfg->base + USB_DEV_MISG3_OFFSET);
	...
	/* Disconnect all EPs from FIFOs */
	sys_write32(USB_BFLB_V2_EPMAP0_NONE, cfg->base + USB_DEV_EPMAP0_OFFSET);
	sys_write32(USB_BFLB_V2_EPMAP1_NONE, cfg->base + USB_DEV_EPMAP1_OFFSET);
	udc_bflb_v2_fifo_setep(dev, USB_BFLB_V2_FIFO_EP_NONE, 1,
			       USB_BFLB_V2_FIFO_DIR_OUT);
	udc_bflb_v2_fifo_setep(dev, USB_BFLB_V2_FIFO_EP_NONE, 2,
			       USB_BFLB_V2_FIFO_DIR_OUT);
	udc_bflb_v2_fifo_setep(dev, USB_BFLB_V2_FIFO_EP_NONE, 3,
			       USB_BFLB_V2_FIFO_DIR_OUT);
	udc_bflb_v2_fifo_setep(dev, USB_BFLB_V2_FIFO_EP_NONE, 4,
			       USB_BFLB_V2_FIFO_DIR_OUT);
...
static int udc_bflb_v2_init(const struct device *const dev)
{
	...
	k_timer_init(&priv->sof_timer, udc_bflb_v2_sof_timer_cb, NULL);
#endif
	...
	if (glb_intstatus & USB_DEV_INT) {
		...
		if (dev_intstatus & USB_INT_G0) {
			...
			if (group_intstatus & USB_CX_SETUP_INT) {
				...
				/* Stop any in-flight CX VDMA from a
				 * previous transfer to prevent stale
				 * data from corrupting the new transfer.
				 */
				tmp = sys_read32(cfg->base +
						 USB_VDMA_CXFPS1_OFFSET);
				tmp &= ~USB_VDMA_START_CXF;
				sys_write32(tmp,
					    cfg->base + USB_VDMA_CXFPS1_OFFSET);

				/* Read setup packet immediately from CX FIFO */
				tmp = sys_read32(cfg->base + USB_DMA_TFN_OFFSET);
				tmp |= USB_ACC_CXF_HOV;
				sys_write32(tmp, cfg->base + USB_DMA_TFN_OFFSET);

				priv->setup_packet[0] = sys_read32(
					cfg->base + USB_DMA_CPS3_OFFSET);
				priv->setup_packet[1] = sys_read32(
					cfg->base + USB_DMA_CPS3_OFFSET);
		...
		if (dev_intstatus & USB_INT_G1) {
			...
			/* F[0..3]_IN_INT signals FIFO-empty, not host
			 * ACK — IN completion is dispatched from G3.
			 */
			for (i = 1U; i <= USB_BFLB_V2_NUM_DATA_FIFOS; i++) {
				if (!(group_intstatus &
				      USB_BFLB_V2_G1_IN_INT(i))) {
					continue;
				}
				tmp = sys_read32(cfg->base +
						 USB_DEV_MISG1_OFFSET);
				tmp |= USB_BFLB_V2_G1_IN_INT(i);
				sys_write32(tmp,
					    cfg->base + USB_DEV_MISG1_OFFSET);
			}
			...
			for (i = 1U; i <= USB_BFLB_V2_NUM_DATA_FIFOS; i++) {
				...
				tmp = sys_read32(cfg->base +
						 USB_DEV_MISG1_OFFSET);
				tmp |= USB_BFLB_V2_G1_OUT_SPK(i);
				sys_write32(tmp,
					    cfg->base + USB_DEV_MISG1_OFFSET);

				/* Read FIFO byte count */
				fibc = sys_read32(cfg->base +
						  USB_DEV_FIBC0_OFFSET +
						  4U * (i - 1U));
				fibc &= USB_BC_F0_MASK; /* byte count, bits [10:0] */

				ep_idx = udc_bflb_v2_fifo_to_ep(dev, i);
				out_cfg = udc_get_ep_cfg(dev, USB_EP_DIR_OUT |
								      ep_idx);
				if (out_cfg == NULL) {
					continue;
				}
				...
				udc_bflb_v2_vdma_startread(
					dev, i, out_buf->data + out_buf->len,
					fibc);
			}
		}
		if (dev_intstatus & USB_INT_G2) {
			...
			if (group_intstatus & USBRST_INT) {
				 ...
				 * reset
				 */
				sys_write32(USB_BFLB_V2_VDMA_CMPLT_ALL,
				...
				priv->reset_expiration = sys_timepoint_calc(
					USB_BFLB_V2_TIMER_AFTER_RESET_T);
			...
			if (group_intstatus & USB_RX0BYTE_INT) {
				...
				for (ep = 1U; ep < USB_BFLB_V2_NUM_BIDIR_EPS;
				     ep++) {
					if (!(rxz & (1U << (ep - 1U)))) {
						continue;
					}
					...
					/* Only submit if the OUT ep is
					 * actually waiting for data.
					 */
					rxz_cfg = udc_get_ep_cfg(
						dev, USB_EP_DIR_OUT | ep);
					if (rxz_cfg != NULL &&
					    udc_ep_is_busy(rxz_cfg)) {
						udc_bflb_v2_ev_submit(
							dev, USB_EP_DIR_OUT | ep,
							UDC_BFLB_V2_EVT_END,
							K_NO_WAIT);
		...
		if (dev_intstatus & USB_INT_G3) {
			...
			for (i = 1U; i <= USB_BFLB_V2_NUM_DATA_FIFOS; i++) {
				...
				/* Stop VDMA before submitting so the remain
				 * count is stable when the work handler reads
				 * it.
				 */
				udc_bflb_v2_vdma_stop(dev, i);
				udc_bflb_v2_ev_submit(dev, dir | ep_idx,
						      UDC_BFLB_V2_EVT_END,
						      K_NO_WAIT);
			}
		}
