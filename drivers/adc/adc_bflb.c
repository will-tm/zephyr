...
/*
 * Fill the sequence buffer with TSEN delta samples. Each output sample is
 * the absolute difference of the averaged TSVBE_LOW=0 and TSVBE_LOW=1
 * phases. The sensor driver converts this delta to a temperature.
 */
static int adc_bflb_read_tsen(const struct device *dev, const struct adc_sequence *sequence)
{
	const struct adc_bflb_config *const cfg = dev->config;
	...
	for (uint32_t i = 0; i < nb_samples; i++) {
		...
		ret = adc_bflb_tsen_phase(dev, false, &v0);
		if (ret < 0) {
			return ret;
		}
		ret = adc_bflb_tsen_phase(dev, true, &v1);
		if (ret < 0) {
			return ret;
		}
		((uint16_t *)sequence->buffer)[i] = (v0 > v1) ? (v0 - v1) : (v1 - v0);
	}
